...
function doCompletion() {
    ...
    var url = "SearchAutoCompleteServlet?action=complete&id=" + escape(completeField.value);
    req = initRequest();
...
function appendProduct(id,name,company) {
    ...
    linkElement.setAttribute("href", "SearchAutoCompleteServlet?action=lookup&id=" + name);
...
function parseMessages(responseXML) {
    ...
    if (responseXML == null) {
        ...
        if (products.childNodes.length > 0) {
            completeTable.setAttribute("bordercolor", "#ff0000");
            completeTable.setAttribute("border", "2");
